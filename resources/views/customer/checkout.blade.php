                            ...
                            <table class="table">
                                    ...
                                    @foreach($cart->items as $item)
                                        ...
                                        <tr class="cart_item" id="cart-item-{{ $product->id }}" data-product-id="{{ $product->id }}" data-price="{{ $price }}">
                                            ...
                                            <td class="cart-product-quantity">
                                                <div class="cart-plus-minus" style="display: flex; align-items: center; border: 1px solid #ddd; border-radius: 4px; overflow: hidden; width: 120px;">
                                                    <div class="dec qtybutton decrease-btn"
                                                         data-product-id="{{ $product->id }}"
                                                         style="width: 35px; height: 38px; background: #f7f7f7; display: flex; align-items: center; justify-content: center; cursor: pointer; font-size: 18px; border-right: 1px solid #ddd;">
                                                    ...
                                                    <input class="cart-plus-minus-box quantity-input"
                                                           ...
                                                           data-product-id="{{ $product->id }}"
                                                           data-price="{{ $price }}"
                                                           style="width: 50px; height: 38px; border: 0; text-align: center; outline: none; font-weight: 600;">
                                                    <div class="inc qtybutton increase-btn"
                                                         data-product-id="{{ $product->id }}"
                                                         style="width: 35px; height: 38px; background: #f7f7f7; display: flex; align-items: center; justify-content: center; cursor: pointer; font-size: 18px; border-left: 1px solid #ddd;">
                                ...
                                <tfoot>
                                    <tr class="cart-subtotal">
                                        ...
                                        <td><span class="amount" id="cartSubtotal">₹{{ number_format($subtotal, 2) }}</span></td>
                                    </tr>
                                    <tr class="order-total">
                                        <th colspan="2">Order Total</th>
                                        <td>
                                            <strong>
                                                <span class="amount" id="cartGrandTotal">₹{{ number_format($subtotal, 2) }}</span>
                                            </strong>
                                        </td>
                                    </tr>
                                </tfoot>
                        ...
                        <div class="payment-method">
                            <div class="payment-accordion">
                                ...
                                <div class="checkout-loader text-center mt-3" id="checkoutLoader" style="display:none;">
                                    <div class="checkout-spinner" style="width:30px;height:30px;border:3px solid #ddd;border-top-color:#2878f0;border-radius:50%;animation:checkoutSpin .7s linear infinite;margin:auto;"></div>
...
<script>
    document.addEventListener('DOMContentLoaded',function(){
        ...
        function updateAllTotalsLocally(){
            ...
            if(grandTotalEl){
                grandTotalEl.textContent=formatPrice(subtotal);
            }
        }
                ...
                items.forEach(function(item){
                    ...
                    if(parseInt(item.product_id)===parseInt(productId)){
                        if(row){
                            ...
                            updateItemTotal(row,itemQuantity);
                        }
                    }
                });
        ...
        document.querySelectorAll('.increase-btn').forEach(function(button){
            button.addEventListener('click',function(e){
                ...
                if(!productId||!input){
                    console.error('Product ID or quantity input not found.');
                    return;
                }
    ...
    if(couponForm){
        couponForm.addEventListener('submit',function(e){
            ...
            .then(result=>{
                ...
                document.getElementById('couponDiscount').textContent=formatPrice(data.discount);

                document.getElementById('cartGrandTotal').textContent=formatPrice(data.total);
            })
            .catch(error=>{
